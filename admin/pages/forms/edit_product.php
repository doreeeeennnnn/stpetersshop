...
if ($db->db_connect()) {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($result && $db->db_count() > 0) {
            // Fetch product data
            $product_data = $db->db_fetch_assoc();

            // Check if product image exists
            if (!empty($product_data['product_img'])) {
                // Set the URL of the product image for preview
                $product_image_url = "../" . $product_data['product_img'];
            } else {
                // If product image does not exist, set a placeholder image URL or leave it blank
                $product_image_url = ""; // Set the placeholder image URL or leave it blank
            }
        } else {
        ...
        // Redirect to products page if product ID is not set
        header("Location: ../../pages/tables/product.php");
        exit();
    }
} else {
                        ...
                        <div class="col-12 grid-margin stretch-card">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form class="forms-sample" method="POST" action="../../back/edit_product.php" enctype="multipart/form-data">
                                        <input type="hidden" name="product_id" value="<?php echo $product_data['product_id']; ?>">
                                        <div class="form-group">
                                            <label for="proname">Product Name</label>
                                            <input type="text" class="form-control" id="proname" placeholder="Name" name="product_name" value="<?php echo $product_data['product_name']; ?>">
                                        </div>
                                        <div class="form-group">
                                            <label for="prodet">Product Details</label>
                                            <input type="text" class="form-control" id="prodet" placeholder="Details" name="product_details" value="<?php echo $product_data['product_details']; ?>">
                                        </div>
                                        <div class="form-group">
                                            <label for="proprice">Product Price</label>
                                            <input type="text" class="form-control" id="proprice" placeholder="Price" name="product_price" value="<?php echo $product_data['product_price']; ?>">
                                        </div>
                                        ...
                                        <div class="mt-2">
                                            <img id="image_preview" src="<?php echo $product_image_url; ?>" alt="Product Image Preview" style="max-width: 200px; max-height: 200px;">
                                        </div>
